    ...
    !trainingLoading && (
      <>
        <div className={`relative ${activeTab === 'calendar' && 'max-h-screen overflow-hidden flex flex-col gap-3'} max-sm:pb-[60px]`}>
          ...
          <ul className='flex font-bold p-3 pb-0'>
            {tabs.map((tab, index) => (
              <li key={index} className={`${activeTab === tab ? 'bg-primary' : 'cursor-pointer'} text-xs rounded-full py-3 px-6 flex gap-3 items-center`} onClick={() => setActiveTab(tab)} >
